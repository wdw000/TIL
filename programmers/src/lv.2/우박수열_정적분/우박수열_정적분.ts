...
function solution(k: number, ranges: number[][]) {
  let y = k;
  const wubak: number[] = [k];
  ...
  for (let i = 1; i < wubak.length; i++) {
    const left = wubak[i - 1];
    const right = wubak[i];

    areaArray.push(((left + right) * 1) / 2);
  }

  ranges.forEach((item) => {
    const start = item[0];
    const end = wubak.length + item[1] - 1;
    const area =
      start > end
        ? -1
        : areaArray
