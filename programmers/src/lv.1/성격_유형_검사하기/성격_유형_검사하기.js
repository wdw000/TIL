...
function solution(survey, choices) {
    ...
    choices.forEach((item, idx) => {
        if (item < 4) {
            result[survey[idx][0]] += point[item - 1];
        }
        else if (item === 0) {
            result[survey[idx][0]] += 0;
            result[survey[idx][1]] += 0;
        }
        else {
            result[survey[idx][1]] += point[item - 1];
        }
    });
    type.forEach((item) => {
        if (result[item[0]] === result[item[1]]) {
            answer.push(item[0] < item[1] ? item[0] : item[1]);
        }
        else {
            answer.push(result[item[0]] > result[item[1]] ? item[0] : item[1]);
        }
    });
    return answer.join("");
}
